...
def mainloop():
    ...
    if window:
        ...
        # 设置区域
        left, top, right, bottom = 2032, 1154, 2236, 1356
        ...
        while True:
            ...
            screenshot2 = capture_window_area(window, 1609, 498, 1704, 565)
            screenshot3 = capture_window_area(window, 1531, 514, 1618, 565)

            # 模板匹配
            max_val, max_loc = match_template(screenshot1, f1)
            print(f"匹配相似度：{max_val}")
            
            if max_val > 0.93:  # 设置阈值
                print("相似度高，点击 F 键")
...
if __name__ == '__main__':
    ...
    if ctypes.windll.shell32.IsUserAnAdmin():
        print('当前已是管理员权限')
        print('是否是3620*2263的有边框窗口模式？')
        print("1. 有边框窗口 2. 3620*2263全屏 3.其他")
        choice = input("请选择：")
        if choice == '1':
            xoffset = 11
            yoffset = 45
