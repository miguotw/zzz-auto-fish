...
def mainloop():
    ...
    if window:
        ...
        while True:
            ...
            screenshot2 = capture_window_area(window, 1620+16, 433-5, 1695+16, 475-5)
            screenshot3 = capture_window_area(window, 1543-4, 439-5, 1614-4, 481-5)
            # 模板匹配
            max_val, max_loc = match_template(screenshot1, f1)
            print(f"F键匹配相似度：{max_val},num={num}")
            ...
            if max(rightshort_val, rightlong_val, leftshort_val, leftlong_val) > 0.4:
                # 取最大值
                max_val = max(rightshort_val, rightlong_val, leftshort_val, leftlong_val)

                if max_val == rightshort_val and rightshort_val > 0.4:
                    print("右短匹配相似度高：连点D键")
                    rapid_click('d', 6, 0.15)

                if max_val == rightlong_val and rightlong_val > 0.4:
                    
                    print("右长匹配相似度高：长按D键")
                    pydirectinput.keyDown('d')  # 长按 'd' 键
                    time.sleep(1.7)  # 长按的时间间隔（2秒）
                    pydirectinput.press('space')
                    pydirectinput.keyUp('d')  # 释放 'd' 键

                if max_val == leftshort_val and leftshort_val > 0.4:
                    print("左短匹配相似度高：连点A键")
                    rapid_click('a', 6, 0.15)

                if max_val == leftlong_val and leftlong_val > 0.4:
